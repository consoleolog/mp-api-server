...
@Query(value = "SELECT p.id, p.title, p.price, p.category, p.description, p.level, p.quizImgName, p.member.id FROM Problem p  WHERE p.category = :category ORDER BY p.id DESC LIMIT 10 OFFSET :offset")
